...
based on: https://github.com/pimoroni/pimoroni-pico/tree/main/micropython/examples/pico_wireless
...
def start_server(http_port=DEFAULT_HTTP_PORT, timeout=5000):
    ...
    picowireless.set_hostname("azpico")
    ...
    while time.ticks_ms() - t_start < timeout:
        ...
        if state == TCP_LISTEN:
            print(f"Server listening on {_ip_str(my_ip)}:{http_port}")
            return server_sock
...
def connect_to_server(host_address, port, client_sock, timeout=5000, connection_mode=TCP_MODE):
    if connection_mode in (TLS_MODE, TLS_BEARSSL_MODE):
        ...
        print(f"Connecting to {_ip_str(host_address)}:{port}...")
...
""".replace("\n", "\r\n")
    ...
    while True:
        if time.ticks_ms() - t_start > timeout:
            ...
            print(f"HTTP request to {host_address}:{port} timed out...")
            return False
    ...
    print(f"Got response: {avail_length} bytes")
